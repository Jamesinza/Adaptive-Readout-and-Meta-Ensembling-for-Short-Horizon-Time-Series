...
apps = ['BETIKA'] # ,'BETGR8','ODIBETS','WINPESA']
...
for APP in apps:
    ...
    test_data1 = df[df['App'] == 'BETGR8'].copy()
    ...
    test_data3 = df[df['App'] == 'WINPESA'].copy()
    ...
    # x1 = keras.layers.GRU(DIM, return_sequences=True, seed=SEED+1)(x_1)
    ...
    x = keras.layers.MultiHeadAttention(num_heads=3, key_dim=6)(fusion, fusion)
    ...
    callbacks = [
        ...
        keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.95, patience=10, cooldown=0),
    ]

    history = model.fit(X_train, y_train, validation_split=0.1, verbose=1, epochs=1000,
