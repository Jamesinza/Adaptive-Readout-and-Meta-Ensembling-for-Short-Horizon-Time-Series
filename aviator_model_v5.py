...
@keras.utils.register_keras_serializable(package="Custom")
class MultiHeadReadout(keras.layers.Layer):
    def __init__(self, dim, num_heads=4):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.mha = keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=dim)
...
@keras.utils.register_keras_serializable(package="Custom")
class GatedResidualPooling(keras.layers.Layer):
    def __init__(self, dim):
        super().__init__()
        self.gate = keras.layers.Dense(dim, activation="sigmoid")
    ...
    def call(self, x):
        ...
        combined = tf.concat([last, avg, mx], axis=-1)  # (B, 3D)

        gate = self.gate(combined)  # (B, D)
        proj = self.proj(combined)  # (B, D)

        # gated mixture
        return gate * proj + (1 - gate) * last


@keras.utils.register_keras_serializable(package="Custom")
class AdvancedGatedReadout(keras.layers.Layer):
    def __init__(self, dim, num_heads=4):
        super().__init__()
        self.dim = dim
    ...
    def call(self, x):
        ...
        # Combine the three
        combined = tf.concat([last, lq_out, mha_out], axis=-1)  # (B, 3D)
        # Gated mixture
        g = self.gate(combined)            # (B,D)
        p = self.proj(combined)            # (B,D)
        return g * p + (1 - g) * last      # gated residual fallback        
...
#         # x: (B, T, D)
#         q = self.query  # (1, D)
#         q = tf.expand_dims(q, axis=1)  # (1, D)
#         q = tf.tile(q, [tf.shape(x)[0], 1])     # (B, D)
#         q = tf.expand_dims(q, 1)               # (B, 1, D)

#         output = self.mha(query=q, value=x, key=x)
#         return tf.squeeze(output, axis=1)      # (B, D)
...
#     x = LearnableQueryPooling(DIM)(x)
#     outputs = keras.layers.Dense(NUM_CLASSES, activation='softmax')(x)
#     model = keras.Model(inputs, outputs)
#     return model

def build_transformer(norm, INPUT_SHAPE, DIM, SEED, NUM_CLASSES, LAYERS):
    inputs = keras.layers.Input(shape=INPUT_SHAPE)
    x = norm(inputs)
    
    x   = keras.layers.Dense(DIM)(x)
    pos = keras_hub.layers.SinePositionEncoding()(x)
    x   = x + pos
...
for APP in apps:
    ...
    model = build_transformer(norm, INPUT_SHAPE, DIM, SEED, NUM_CLASSES, LAYERS)
    model.summary()
    ...
    model.fit(X_train, y_train, validation_split=0.1, verbose=1, epochs=EPOCHS,
               batch_size=BATCH_SIZE, callbacks=callbacks, class_weight=class_weights_dict,
               shuffle=False,
                       )
...
meta_model.compile(optimizer=keras.optimizers.AdamW(5e-4), loss='sparse_categorical_crossentropy')

callbacks = [
    keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True, verbose=1),
    keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.9, patience=1, cooldown=0, min_lr=1e-8),
]

meta_model.fit(reshaped_new_preds, y_test, validation_split=0.1, shuffle=True, batch_size=128,
